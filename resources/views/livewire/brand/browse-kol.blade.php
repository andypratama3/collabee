<div>
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Browse KOL</h1>
            <p class="mt-1 text-sm text-gray-500">Find the perfect KOL for your campaign</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div>
                <select wire:model.live="category" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
        ...
        <div class="mt-3 flex items-center gap-4">
            <label class="flex items-center gap-2 text-sm text-gray-600">
                <input type="checkbox" wire:model.live="openForWork" class="rounded border-gray-300 text-indigo-600">
                Open for work only
            </label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($kols as $kol)
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                <div class="flex items-start gap-4">
                    <div class="w-14 h-14 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-lg flex-shrink-0">
                        {{ strtoupper(substr($kol->display_name, 0, 2)) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-gray-900 truncate">{{ $kol->display_name }}</h3>
                        <p class="text-sm text-gray-500">{{ $kol->category ? ucfirst($kol->category) : 'General' }}</p>
                        ...
                        @if($kol->location)
                            <p class="text-sm text-gray-400 mt-1">{{ $kol->location }}</p>
                        @endif
        ...
        @empty
            <div class="col-span-full text-center py-12">
                <p class="text-gray-500">No KOL found matching your criteria.</p>
            </div>
        @endforelse
    </div>
    ...
    @if($showHireModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" wire:click.self="$set('showHireModal', false)">
            <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg mx-4">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Hire {{ $selectedKolName }}</h3>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Campaign *</label>
                        <select wire:model="hireCampaignId" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
