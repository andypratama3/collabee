                ...
                @foreach(['Campaign Info', 'Specifications', 'Brief', 'Budget & Schedule'] as $i => $label)
                    <div class="flex items-center shrink-0">
                        <div class="flex items-center justify-center w-8 h-8 rounded-full text-sm font-medium
                            {{ $step > $i + 1 ? 'bg-indigo-600 text-white' : ($step === $i + 1 ? 'bg-indigo-600 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-400') }}">
                            {{ $step > $i + 1 ? '✓' : $i + 1 }}
                        </div>
                        <span class="ml-2 text-sm {{ $step === $i + 1 ? 'text-indigo-600 font-medium' : 'text-gray-500 dark:text-gray-400' }} hidden sm:inline">{{ $label }}</span>
                        @if($i < 3)
                            <div class="w-12 h-0.5 mx-2 {{ $step > $i + 1 ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-gray-600' }}"></div>
                        @endif
                    </div>
                @endforeach
            </div>
            ...
            <div class="flex items-center gap-3">
                @if($step === 4)
                    <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                        <input type="checkbox" wire:model="saveAsDraft" class="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-indigo-600">
                        Save as draft
                    </label>
