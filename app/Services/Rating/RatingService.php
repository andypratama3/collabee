...
class RatingService
{
    public function rateKol(Hiring $hiring, User $rater, array $data): Rating
    {
        return DB::transaction(function () use ($hiring, $rater, $data) {
            // Prevent duplicate
            if (Rating::where('hiring_id', $hiring->id)->where('rater_id', $rater->id)->exists()) {
                throw new \RuntimeException('Anda sudah memberikan rating untuk hiring ini.');
            }
            ...
            $hiring->load('kolProfile');

            if (! $hiring->kolProfile) {
                throw new \RuntimeException('KOL profile tidak ditemukan.');
            }
            ...
            $profile->update([
                'rating_avg'   => Rating::where('rated_id', $profile->user_id)->where('type', 'kol')->avg('overall'),
                'rating_count' => Rating::where('rated_id', $profile->user_id)->where('type', 'kol')->count(),
            ]);
    ...
    public function rateBrand(Hiring $hiring, User $rater, array $data): Rating
    {
        return DB::transaction(function () use ($hiring, $rater, $data) {
            ...
            $hiring->load('brandProfile');

            if (! $hiring->brandProfile) {
                throw new \RuntimeException('Brand profile tidak ditemukan.');
            }
            ...
            $profile->update([
                'rating_avg'   => Rating::where('rated_id', $profile->user_id)->where('type', 'brand')->avg('overall'),
                'rating_count' => Rating::where('rated_id', $profile->user_id)->where('type', 'brand')->count(),
            ]);
